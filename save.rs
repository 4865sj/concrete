...
fn main() {   
    ...
    let clear_a = test_a.floor() as u8;
    let clear_b = test_b.floor() as u8;
   ...
   */
   ...
   /* load and calaulate 
    ...
    let start3 = Instant::now();
    let mut new_ciphertext: Vec<FheUint8> = read_ciphertext();
    let duration3 = start3.elapsed();
    ...
    let after_a: u8 = new_a.decrypt(&new_client_key);
    let after_b: u8 = new_b.decrypt(&new_client_key);
    let after_result: u8 = result.decrypt(&new_client_key);
